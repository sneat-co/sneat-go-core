...
func GenerateIDFromNameOrRandom(name *Name, existingIDs []string) (id string, err error) {
	...
	//
	if nick := CleanForID(trans.Transliterate(name.Nick, "")); nick != "" {
		if !slice.Contains(existingIDs, id) {
			return nick, nil
		}
	}
	...
	if first == "" || last == "" || middle == "" {
		if first == "" && last == "" && middle == "" {
			if name.Full != "" {
				if names := strings.Split(name.Full, " "); len(names) > 0 {
					...
					if len(id) > 0 && !slice.Contains(existingIDs, id) {
						return id, nil
					}
					if len(names) == 2 {
						first = names[0]
		...
		if first != "" && last != "" {
			...
			if id = first[0:1] + last[0:1]; !slice.Contains(existingIDs, id) {
				return id, nil
			}
		}
		if first != "" && middle != "" && last != "" {
			// Try to user 1st chars of all first, middle, last names
			if id = first[0:1] + middle[0:1] + last[0:1]; !slice.Contains(existingIDs, id) {
				return id, nil
			}

			// Try to user 1st chars of all first, last names, middle names
			if id = first[0:1] + last[0:1] + middle[0:1]; !slice.Contains(existingIDs, id) {
				return id, nil
			}
		}
		...
		if first != "" {
			...
			// Try to use 1st char of first name
			if id = first[0:1]; !slice.Contains(existingIDs, id) {
				return id, nil
			}

			// Try to use 1st and last char of first name
			if id = first[0:1] + first[len(first)-1:]; slice.Contains(existingIDs, id) {
				return id, nil
			}

			// Try to use the whole first name
			if id = first; !slice.Contains(existingIDs, id) {
				return id, nil
			}
		}
		if first != "" && last != "" {
			// Try to use full first name and 1st char of last name
			if id = first + last[0:1]; slice.Contains(existingIDs, id) {
				return id, nil
			}
		}
		if last != "" {
			if id = last; slice.Contains(existingIDs, id) {
				return id, nil
			}
		}
	}
